...
#
#         tracks = new_trajectories
#
#         cv2.polylines(img, [np.int32(trajectory) for trajectory in trajectories], False,)
#         cv2.putText((img, 'track count: %d' % len(trajectories), (20, 50), cv2.FONT_HERSHEY_DUPLEX))
#
...
# Check if video capture has been initialized
if not cap.isOpened():
...
while True:
    ...
    if len(trajectories) > 0:
        ...
        cv2.putText(img, 'Track count: %d' % len(trajectories), (20, 50), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 0, 0), 1)
